  ...
  {isLastRow
    ? (() => {
        ...
        if (!isNaN(fromTime.getTime()) && !isNaN(currentTime.getTime())) {
          const timeDiffInMinutes = Math.floor(
            (currentTime - fromTime) / (60 * 1000)
          );
          // console.log("timeDiffInMinutes:", timeDiffInMinutes);

          return timeDiffInMinutes >= 0 ? `${timeDiffInMinutes}` : '0';
        } else {
          // console.error("Invalid date format. Trying to parse...");
          
          // Try parsing the date with a different format
          const parsedFromTime = moment(item.FromTime, "DD/MM/YYYY HH:mm:ss", true);
          if (parsedFromTime.isValid()) {
            const timeDiffInMinutes = Math.floor(
              (currentTime - parsedFromTime.toDate()) / (60 * 1000)
            );
            // console.log("timeDiffInMinutes:", timeDiffInMinutes);

            return timeDiffInMinutes >= 0 ? `${timeDiffInMinutes}` : '0';
          } else {
            // console.error("Failed to parse date. Returning N/A.");
            return '0';
          }
        }
      })()
